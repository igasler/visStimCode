...
The behaviour of PTB can be controlled by the command:
  ...
  -1 = Disable all cleverness, take noisy timestamps. This is the behaviour
   ...
   3 = Always use kernel-level timestamping, fall back to noisy stamps if it fails.
    ...
    % Screen parameters:
    screenSize = 60;              % x screen size in centimeters 58
    mouseDistancecm = 30;           % mouse distance from the screen im cm 20
    ...
    space_freq_deg = 0.035 ;              % spatial frequency in 1/pixels 0.035
    ...
    orientation_time = 3;           % time in seconds after the orientation changes 3
    ...
    mouseDistance = fix((screenRect(3) / screenSize) * mouseDistancecm);           % in pixel
     ...
     %   space_freq = 1 / ( 2 * mouseDistancecm * tan( ( ( 1 / space_freq_deg ) * pi / 180 ) / 2 ) * screenRect(3) / screenSize );   % edit jleong 050718
    ...
    n_repetitions = 6; %6             % number of stim cycles
            ...
            for s=1: orientations;
                ...
                if j*eye*s>1;
                    ...
                    if set_trigger
                        ...
                        toclist(s,j,1)=toc; % from last trigger to the end of gray screen
                        toclist2(s,j,1)=toc(t1); % from last trigger to the end of gray screen
                        tic
                    else
                        tic
                        %while ~KbCheck;end
                ...
                while toc-toclist(s,j,2)<(patch_delay)
                    
                    Screen('FillRect', window,127);
                    Screen('Flip',window);
                    
                    if  ceil((toc-toclist(s,j,2))*hz)==ceil((patch_delay-1.9)*hz)% w==ceil((patch_delay-1)*hz)%-1
                    ...
                    if ceil((toc-toclist(s,j,2))*hz)==ceil((patch_delay-1.4)*hz)%w==ceil((patch_delay-0.1)*hz)%-0.5
                        putsample(AO,[0 0]);
                        pause(0.05)
                        putsample(AO,[5 0]);
                ...
                toclist(s,j,2)=toc; % from last trigger to drifting grating
                toclist2(s,j,2)=toc(t1); % from last trigger to the end of gray screen
                                                      
                while toc-toclist(s,j,2)<(patch_time)
...
%                    rand_log.repetition(j).eye(eye).patches(s).timing(i)=toc;
                ...
                toclist(s,j,3)=toc; % from last trigger to the end of drifting gratting
                toclist2(s,j,3)=toc(t1);
